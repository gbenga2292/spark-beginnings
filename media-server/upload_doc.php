...
$journal_id = trim($_POST['journal_id'] ?? ''); // match exact empty-string fallback
$site_name  = trim($_POST['site_name'] ?? '');
$asset_id   = trim($_POST['asset_id'] ?? '0');
...
// ── Validate File is a Document (PDF, DOC, DOCX) ────────────────────────────
$ext  = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));

$allowed_exts = ['pdf', 'doc', 'docx'];
if (!in_array($ext, $allowed_exts)) {
    http_response_code(400);
    echo json_encode(['error' => 'Invalid file extension. Only PDF and Word Documents (.doc, .docx) are allowed.']);
    exit;
}

$file_type = 'image';

// ── Resolve asset_name (caption) fallback ─────────────────────────────────────
if ($asset_name === '') {
    $asset_name = 'Site Document';
}
...
try {
    ...
    if ($column_check) {
        $stmt = $pdo->prepare("INSERT INTO site_media
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$site_id, $journal_id, $site_name, $asset_id, $asset_name, $log_date, $filename, $file_type, $dest, $url, $user_id, $user_name]);
    } else {
        $stmt = $pdo->prepare("INSERT INTO site_media
            (site_id, site_name, asset_id, asset_name, log_date, file_name, file_type, file_path, url, uploaded_by, uploaded_by_name)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$site_id, $site_name, $asset_id, $asset_name, $log_date, $filename, $file_type, $dest, $url, $user_id, $user_name]);
    }
