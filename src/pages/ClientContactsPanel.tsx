...
export function ClientContactsPanel({ clientName, onClose, inline = false, siteId }: Props) {
  ...
  // Existing contacts for this client not yet linked to this site and not principal
  const unlinkedContacts = siteId
    ? allClientContacts.filter(c => !c.isPrincipal && !(c.siteIds || []).includes(siteId))
    : [];

  const currentSite = clientSites.find(s => s.id === siteId);

  const handleLinkExisting = (c: ClientContact) => {
    if (!siteId) return;
    const currentSiteName = currentSite?.name || 'this site';
    const newSiteIds = [...(c.siteIds || []), siteId];
    const newSiteNames = [...(c.siteNames || []), currentSiteName];
    updateClientContact(c.id, {
      siteIds: newSiteIds,
      siteNames: newSiteNames,
    });
    toast.success(`"${c.name}" added to ${currentSiteName}`);
  };
  ...
  const bodyContent = (
    <>
      ...
      {!showAdd && !showLinkExisting && editingId === null && (
        <div className={cn("grid gap-2", siteId && unlinkedContacts.length > 0 ? "grid-cols-2" : "grid-cols-1")}>
          <Button
            onClick={() => {
              setShowAdd(true);
              setDraft({
                ...
                siteNames: currentSite ? [currentSite.name] : [],
              });
            }}
            variant="outline"
            className={cn('gap-2 h-9 text-xs border-dashed', isDark ? 'border-slate-600 text-slate-400 hover:bg-slate-800' : 'border-slate-300 text-slate-500 hover:bg-slate-50')}
          >
            <Plus className="w-3.5 h-3.5" /> New Contact
          </Button>

          {siteId && unlinkedContacts.length > 0 && (
            <Button
              onClick={() => setShowLinkExisting(true)}
              variant="outline"
              className={cn('gap-2 h-9 text-xs border-blue-300 text-blue-600 hover:bg-blue-50 dark:border-blue-800 dark:text-blue-400 dark:hover:bg-blue-950/40 font-semibold')}
            >
              <UserCheck className="w-3.5 h-3.5" /> Add from Directory ({unlinkedContacts.length})
            </Button>
          )}
        </div>
      )}

      {/* Panel for linking existing directory contacts to this site */}
      {showLinkExisting && (
        <div className={cn('rounded-xl border p-4 space-y-3', isDark ? 'bg-slate-800 border-slate-700' : 'bg-blue-50/50 border-blue-200')}>
          <div className="flex items-center justify-between">
            <div>
              <h3 className={cn('text-xs font-bold uppercase tracking-wider', isDark ? 'text-slate-200' : 'text-slate-800')}>
                Add Existing Contact to {currentSite?.name || 'this Site'}
              </h3>
              <p className={cn('text-[11px] mt-0.5', isDark ? 'text-slate-400' : 'text-slate-500')}>
                Select a contact from {clientName}&apos;s directory to add to this site.
              </p>
            </div>
            <Button variant="ghost" size="sm" onClick={() => setShowLinkExisting(false)} className="h-7 text-xs px-2.5">
              <X className="w-3.5 h-3.5" />
            </Button>
          </div>

          <div className="space-y-2 max-h-60 overflow-y-auto pr-1 style-scroll">
            {unlinkedContacts.map(c => (
              <div
                key={c.id}
                className={cn('rounded-lg border p-2.5 flex items-center justify-between gap-2', isDark ? 'bg-slate-900/60 border-slate-700' : 'bg-white border-slate-200')}
              >
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className={cn('text-xs font-bold truncate', isDark ? 'text-slate-100' : 'text-slate-800')}>{c.name}</span>
                    {c.position && (
                      <span className={cn('text-[10px] truncate', isDark ? 'text-slate-400' : 'text-slate-500')}>• {c.position}</span>
                    )}
                  </div>
                  ...
                  {(c.siteNames?.length ?? 0) > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
                      <span className="text-[10px] text-slate-400">On other sites:</span>
                      {c.siteNames!.map((sn, idx) => (
                        <span key={idx} className={cn('text-[9px] px-1 py-0.2 rounded', isDark ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-600')}>
                          {sn}
                        </span>
                      ))}
                    </div>
                ...
                <Button
                  ...
                  className="h-7 text-xs px-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg shrink-0 gap-1 font-semibold"
                >
                  <Plus className="w-3 h-3" /> Add to Site
                </Button>
              </div>
            ))}
          </div>
      ...
      {/* Site-scoped notice */}
      {siteId && !showAdd && !showLinkExisting && editingId === null && (
        <div className={cn('flex items-center gap-2 text-xs px-3 py-2 rounded-lg border', isDark ? 'bg-slate-800 border-slate-700 text-slate-400' : 'bg-blue-50 border-blue-100 text-blue-600')}>
          ...
          {unlinkedContacts.length > 0 && (
            <button
              type="button"
              onClick={() => setShowLinkExisting(true)}
              className="ml-auto font-semibold underline hover:text-blue-700 dark:hover:text-blue-300 transition-colors cursor-pointer"
            >
              {unlinkedContacts.length} in directory (click to add)
            </button>
          )}
        </div>
      )}
