      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm shrink-0">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xl font-bold text-slate-900">
            {isEditing ? 'Due Process Review & Update' : 'Log Due Process / Event'}
          </h2>
        </div>
        ...
        <Card className="shadow-sm border-slate-200">
          ...
          <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-2 gap-5 px-6 pb-6">
            ...
            <div className="space-y-2">
              ...
              <select className="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm focus:ring-rose-500/20" value={formData.type} onChange={e => setFormData({ ...formData, type: e.target.value })}>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-wider text-slate-500">Reported Severity</label>
              <select className="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm focus:ring-rose-500/20" value={formData.severity} onChange={e => setFormData({ ...formData, severity: e.target.value })}>
    ...
    <div className="flex flex-col h-[calc(100vh-6rem)] gap-4 relative">
      {!(isAdding || isEditing) && (
        <div className="flex justify-between items-end">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900 bg-clip-text text-transparent bg-gradient-to-r from-rose-700 to-rose-400">
              Disciplinary Center
            </h1>
            <p className="text-sm font-medium text-slate-500 mt-1">Due process execution, active directory, and query monitoring.</p>
          </div>
          <div className="flex items-center gap-2">
                ...
                .map(emp => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {hasActiveEvent && (
                          <div className="h-2 w-2 rounded-full bg-rose-500 shadow-sm shrink-0" title="Active Disciplinary Action"></div>
                      )}
                    </div>
                  )
                })
          ...
          {showNotices && !isEditing ? (
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                <div className="bg-white border-b border-slate-200 p-6 flex justify-between items-center shrink-0 shadow-sm">
                  ...
                  {priv.canAdd && (
                      <Button className="bg-rose-600 hover:bg-rose-700 shadow-sm px-6" onClick={() => { setFormData({...emptyForm, employeeId: selectedEmployeeId}); setIsAdding(true); }}>
                        <Plus className="h-4 w-4 mr-2" /> Log Due Process Setup
                      </Button>
                  )}
                </div>
