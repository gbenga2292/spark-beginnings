...
import { 
  ...
  Search, Filter, Calendar, ChevronRight, Info, CheckCircle2, MessageSquare
} from "lucide-react";
...
// Zero-re-render breakpoint detection
...
export function TaskArchive() {
  ...
  useSetPageTitle(
    "Task Archive",
    `Items here are kept for ${retentionDays} days before final deletion`,
  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto min-h-full">
      <div className="flex items-center gap-1 mb-6 bg-muted/50 p-1 rounded-xl w-full sm:w-fit">
        <button 
          onClick={() => setActiveTab('subtasks')}
          className={`flex-1 sm:flex-none px-4 py-2.5 sm:py-2 text-sm font-semibold rounded-lg transition-all ${activeTab === 'subtasks' ? 'bg-card shadow-sm text-primary' : 'text-muted-foreground hover:text-foreground'}`}
        >
          Subtasks ({archivedSubs.length})
        </button>
        <button 
          onClick={() => setActiveTab('main_tasks')}
          className={`flex-1 sm:flex-none px-4 py-2.5 sm:py-2 text-sm font-semibold rounded-lg transition-all ${activeTab === 'main_tasks' ? 'bg-card shadow-sm text-primary' : 'text-muted-foreground hover:text-foreground'}`}
        >
      ...
      {loading ? (
        ...
        <div className="bg-card border border-border rounded-2xl p-8 sm:p-20 text-center shadow-sm">
          <div className="w-14 h-14 sm:w-16 sm:h-16 bg-muted rounded-full flex items-center justify-center mx-auto mb-4">
            <CheckCircle2 className="w-7 h-7 sm:w-8 sm:h-8 text-muted-foreground/40" />
          </div>
          <h3 className="text-base sm:text-lg font-bold text-foreground">Archive is empty</h3>
          <p className="text-xs sm:text-sm text-muted-foreground mt-1">No {activeTab === 'subtasks' ? 'subtasks' : 'tasks'} found in the archive.</p>
        </div>
      ) : (
            ...
            {(activeTab === 'subtasks' ? filteredSubs : filteredMain).map((item: any) => {
              ...
              return (
                  <motion.div 
                    ...
                    className="group bg-card border border-border rounded-2xl p-3.5 sm:p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 hover:shadow-md transition-all hover:border-primary/20"
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center flex-wrap gap-2 mb-1.5 sm:mb-1">
                        <span className="text-sm sm:text-base font-bold text-foreground truncate">{item.title}</span>
                        {activeTab === 'subtasks' && parentTask && (
                          <span className="text-[10px] bg-muted/80 px-2 py-0.5 rounded-full text-muted-foreground border border-border/50 truncate max-w-full">
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] sm:text-[11px]">
                        ...
                        <span className="flex items-center gap-1.5 text-muted-foreground">
                          <MessageSquare className="w-3 h-3" /> {
                            comments.filter(c => 
                              activeTab === 'main_tasks' 
                                ? (c.main_task_id === item.id || c.task_id === item.id)
                                : (c.subtask_id === item.id || c.task_id === item.id)
                            ).length
                          } updates
                        </span>
                    ...
                    <div className="flex items-center gap-2 sm:opacity-0 sm:group-hover:opacity-100 transition-all">
                      ...
                      <Button 
                        ...
                        className="h-9 w-9 sm:h-8 sm:w-8 p-0 text-rose-500 hover:text-rose-600 hover:bg-rose-50"
                      >
      ...
      <div className="mt-8 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30 rounded-2xl p-4 sm:p-5 flex gap-3 sm:gap-4">
        <div className="w-9 h-9 sm:w-10 sm:h-10 bg-amber-100 dark:bg-amber-900/30 rounded-xl flex items-center justify-center flex-shrink-0">
          <Info className="w-4 h-4 sm:w-5 sm:h-5 text-amber-600" />
        </div>
        <div>
          <h4 className="text-xs sm:text-sm font-bold text-amber-900 dark:text-amber-100 uppercase tracking-wide">About the Task Archive</h4>
          <p className="text-[10px] sm:text-xs text-amber-800/70 dark:text-amber-100/60 mt-1 leading-relaxed">
            When you delete a task or subtask, it stays here for {retentionDays} days, 
            allowing you to restore it if needed. After this period, it is permanently removed.
            Change the retention period in <b>Settings</b>.
          </p>
        </div>
      </div>
    </div>
