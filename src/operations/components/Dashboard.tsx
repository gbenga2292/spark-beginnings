      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className={cn(
          ...
          <div className="px-6 py-5 border-b flex items-center justify-between">
            <h3 className="font-bold text-lg text-slate-900">Recent Logistics Activity</h3>
            ...
            {waybills.slice(0, 5).map((wb) => (
              <div key={wb.id} className="px-6 py-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-bold text-slate-900">{wb.siteName}</p>
                    <p className="text-[10px] text-slate-400 font-medium flex items-center gap-1 uppercase tracking-tight">
                ...
                <div className="flex items-center gap-6">
                  <span className={cn(
                    "px-3 py-1 rounded-full text-[9px] font-black uppercase border-0",
                    wb.status === 'sent_to_site' ? "bg-blue-100 text-blue-700" : "bg-orange-100 text-orange-700"
                  )}>
                    {wb.status.replace(/_/g, ' ')}
                  </span>
                  <div className="text-right">
                    <p className="text-sm font-black text-slate-900">{wb.items.length}</p>
        ...
        <div className={cn(
          ...
          isDark ? "bg-slate-900 border-slate-800" : "bg-white"
           ...
           <h3 className="font-bold text-lg mb-6 text-slate-900">Asset Distribution</h3>
           ...
           <div className="mt-8 pt-6 border-t border-slate-50">
              <button className="w-full flex items-center justify-center gap-2 py-4 px-4 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition-colors text-xs font-black uppercase text-slate-600 tracking-widest border border-slate-100">
