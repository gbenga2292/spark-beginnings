...
function ConsensusDashboard({ sessionId, evaluations, users, currentUser, onConsensusSubmit }: { sessionId: string, evaluations: any[], users: AppUser[], currentUser: any, onConsensusSubmit: (data: any) => void }) {
  const panelEvals = evaluations?.filter((e: any) => e.sessionId === sessionId && e.evaluationRole === 'PANELIST') || [];
  const existingConsensus = evaluations?.find((e: any) => e.sessionId === sessionId && e.evaluationRole === 'CONSENSUS');
  
  const [conclusion, setConclusion] = useState(existingConsensus?.type || 'Confirm Employment');
  const [score, setScore] = useState(existingConsensus?.overallScore?.toString() || '0');
  const [notes, setNotes] = useState(existingConsensus?.managerNotes || '');

  const isLocked = !!existingConsensus;

  if (panelEvals.length === 0) return null;
    ...
    <div className="mt-8 pt-8 border-t border-slate-200">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2">
          <Presentation className="w-5 h-5 text-indigo-500" /> Consensus Dashboard
        </h3>
        {isLocked && <div className="text-xs font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200 flex items-center gap-1"><CheckCircle2 className="w-4 h-4"/> Validated & Locked</div>}
      </div>

      {/* Comparison View */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        {panelEvals.map((p: any) => (
          <div key={p.id} className="bg-slate-50 border border-slate-200 rounded-xl p-4 shadow-sm">
            <div className="flex justify-between items-start mb-2">
              <div className="font-bold text-slate-700 text-sm flex items-center gap-2">
                <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-[10px] text-slate-500">{resolveName(p.createdBy).substring(0,2).toUpperCase()}</div>
                {resolveName(p.createdBy)}
              </div>
              <span className={`font-bold text-sm ${p.overallScore >= 70 ? 'text-emerald-600' : 'text-indigo-600'}`}>{p.overallScore}%</span>
            </div>
            <p className="text-xs text-slate-600 leading-relaxed italic border-l-2 border-indigo-200 pl-3 mt-3">"{p.managerNotes}"</p>
          </div>
        ))}
      </div>

      {/* Final Consensus Form */}
      <div className="bg-indigo-50/50 border border-indigo-100 rounded-2xl p-6">
        <h4 className="text-xs font-bold text-indigo-800 uppercase tracking-wider mb-4">Final Deliberation & Conclusion</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Panel Conclusion</label>
            <select 
              value={conclusion} onChange={e => setConclusion(e.target.value)} disabled={isLocked}
              className="w-full text-sm rounded-lg border-slate-200 bg-white p-2.5 outline-none focus:border-indigo-500 disabled:opacity-60 disabled:bg-slate-50"
            >
              ...
              <option value="End with Salary Increase">Confirm with Salary Increase</option>
            </select>
          </div>
          <div>
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Agreed Final Score (%)</label>
            <input 
              type="number" value={score} onChange={e => setScore(e.target.value)} disabled={isLocked}
              className="w-full text-sm rounded-lg border-slate-200 bg-white p-2.5 outline-none focus:border-indigo-500 disabled:opacity-60 disabled:bg-slate-50"
              placeholder="e.g. 85" max="100" min="0"
            />
          </div>
        </div>
        <div className="mb-5">
          <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Final HR/Admin Notes</label>
          <textarea 
            value={notes} onChange={e => setNotes(e.target.value)} disabled={isLocked}
            className="w-full text-sm rounded-lg border-slate-200 bg-white p-3 outline-none focus:border-indigo-500 min-h-[100px] resize-none disabled:opacity-60 disabled:bg-slate-50"
            placeholder="Summarize the panel's final decision reasoning here..."
          />
        </div>
        ...
        {!isLocked && (
          <div className="flex justify-end">
            <button 
              onClick={() => onConsensusSubmit({ conclusion, score: Number(score), notes })}
              disabled={!conclusion || !score}
              className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-xl shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
              <CheckCircle2 className="w-4 h-4" /> Validate & Lock Panel
            </button>
          </div>
        )}
      </div>
...
export function TaskInboxView({ subtasks, mainTasks, users, activeSubtaskId, onSelectSubtask, className, onClose }: TaskInboxViewProps) {
  ...
  // Custom Evaluation UI Prompt State
  const [evalPrompt, setEvalPrompt] = useState<{ isOpen: boolean; subtaskId: string | null; sessionId: string; employeeName: string; employeeId: string; score: string; notes: string; id?: string; conclusion: string }>({
              ...
              <div className="p-4 md:p-8 max-w-3xl">
                ...
                <div className="flex items-center gap-3 flex-wrap mb-8">
                  ...
                  {(() => {
                    ...
                    let isPanelReview = false;
                    let panelMeta: any = null;
                    try {
                      const meta = JSON.parse(activeSubtask.description || '{}');
                    ...
                    if (isPanelReview && panelMeta) {
                      const isConsensusLocked = evaluations?.some(e => e.sessionId === panelMeta.sessionId && e.evaluationRole === 'CONSENSUS') || false;
                      const existingEval = evaluations?.find(e => e.sessionId === panelMeta.sessionId && e.evaluationRole === 'PANELIST' && (e.createdBy === currentUser?.id || e.createdBy === subUser?.name));
                      const isAssignee = activeSubtask.assignedTo?.split(',').includes(currentUser?.id || '');
                      const hasActed = activeSubtask.status === 'completed' || !!existingEval;

                      return (
                        <button
                          onClick={() => {
                            if (isConsensusLocked || !isAssignee) return;
                            const empName = panelMeta.employeeName || 'Employee';
                            const cleanEmpName = empName?.toLowerCase().trim() || '';
                            const empIdMatch = employees.find(e => 
                              (e.firstname?.trim() + ' ' + e.surname?.trim()).toLowerCase() === cleanEmpName ||
                              (e.surname?.trim() + ' ' + e.firstname?.trim()).toLowerCase() === cleanEmpName ||
                              e.id === empName
                            );
                            setEvalPrompt({
                              isOpen: true,
                              ...
                              sessionId: panelMeta.sessionId,
                              employeeName: empName,
                              employeeId: empIdMatch ? empIdMatch.id : '00000000-0000-0000-0000-000000000000',
                              score: existingEval?.overallScore?.toString() || '80',
                              notes: existingEval?.managerNotes || '',
                              conclusion: existingEval?.type || 'Confirm Employment'
                            });
                          }}
                          disabled={isConsensusLocked || !isAssignee}
                          className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold border transition-all ${
                            isConsensusLocked ? 'bg-slate-50 text-slate-400 border-slate-200 cursor-not-allowed' :
                            !isAssignee ? 'bg-slate-50 text-slate-300 border-slate-100 cursor-not-allowed hidden' :
                            hasActed ? 'bg-white text-indigo-600 border-indigo-200 hover:bg-indigo-50 shadow-sm' :
                            'bg-indigo-600 text-white border-indigo-600 hover:bg-indigo-700 shadow-sm'
                          }`}
                        >
                          <Presentation className="w-4 h-4" /> {isConsensusLocked ? 'Panel Locked' : hasActed ? 'Edit Evaluation' : 'Submit Evaluation'}
                        </button>
                      );
                    }
                  ...
                  {/* Panel Invitation for Probation Tasks */}
                  {(() => {
                    const isHr = subUser?.department?.toLowerCase() === 'hr' || subUser?.privileges?.tasks?.isExternalHr;
                    const isAdmin = currentUser?.role === 'admin' || currentUser?.role === 'co-admin';
                    if ((!isHr && !isAdmin) || activeSubtask.status === 'completed') return null;

                    return (
                      <button
                        onClick={() => setPanelInvite({ isOpen: true, panelists: [] })}
                        className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold bg-indigo-600 text-white border border-indigo-600 hover:bg-indigo-700 shadow-sm transition-all"
                      >
                        <Users className="w-4 h-4" /> Invite Panel
                      </button>
                    );
                  })()}
                </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          ...
          >
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <h3 className="font-bold text-slate-800">Invite Evaluation Panel</h3>
              <button onClick={() => setPanelInvite({ isOpen: false, panelists: [] })} className="text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6">
                  ...
                  return (
                    <div 
                      ...
                      onClick={() => {
                        setPanelInvite(prev => ({
                          ...prev,
                          panelists: isSelected ? prev.panelists.filter(id => id !== u.id) : [...prev.panelists, u.id]
                        }));
                      }}
                      className={`flex items-center gap-3 p-2.5 rounded-xl border cursor-pointer transition-all ${
                        isSelected ? 'bg-indigo-50 border-indigo-200 ring-1 ring-indigo-200' : 'hover:bg-slate-50 border-slate-100'
                      }`}
                    >
                      <Avatar className="w-8 h-8">
                        <AvatarFallback className={`text-[10px] text-white ${u.avatarColor || 'bg-slate-500'}`}>
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              <button className="px-4 py-2 text-sm font-semibold rounded-lg text-slate-600 hover:bg-slate-100 transition-colors" onClick={() => setPanelInvite({ isOpen: false, panelists: [] })}>Cancel</button>
              <button 
                disabled={panelInvite.panelists.length === 0}
                className="px-4 py-2 text-sm font-semibold rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                onClick={() => {
                  const sessionId = crypto.randomUUID();
                  panelInvite.panelists.forEach(userId => {
                    addSubtask({
                      mainTaskId: activeMainTaskId!,
                      title: `Panel Review: ${activeMainTask?.title?.split(': ')[1] || 'Employee'}`,
                      description: JSON.stringify({
                        refType: 'probation_panel',
                        sessionId,
                        narration: `Panelist evaluation requested for ${activeMainTask?.title?.split(': ')[1] || 'this employee'}.`,
                        role: 'PANELIST',
                        employeeName: activeMainTask?.title?.split(': ')[1] || 'Employee'
                      }),
                      status: 'not_started',
                      assignedTo: userId,
                      priority: 'high',
